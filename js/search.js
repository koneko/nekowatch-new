...
var interval = setInterval(function () {
    ...
    if (params.get("q")) {
        ...
        search("invalid", "searchres", params.get("q"), override);
    } else {
        popular("searchres")
    }
    ...
    window.specialty = querySearch
    document.getElementById("submitbtn").onclick = querySearch
    async function search (inid, outid, override, or) {
        ...
        let data = await window.gogo.search(qq)
        console.log(data);
        output.innerHTML = "";
        data.forEach((item) => {
            ...
            div.innerHTML = `
                    ...
                    <div>
                    <h3><a href="/anime?q=${title}">${item.title
                }</a></h3>
                    </div>
                    `;
            div.setAttribute(
    ...
    async function popular (outid) {
        ...
        let data = await window.gogo.newEpisodes()
        console.log(data);
        output.innerHTML = "";
        data.forEach((item) => {
